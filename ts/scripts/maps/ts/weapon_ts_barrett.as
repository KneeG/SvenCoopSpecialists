//////////////////////////////////////////////////////////
// File         : weapon_ts_barrett.as                  //
// Author       : Knee                                  //
// Description  : BARRETT from The Specialists Mod 3.0  //
//////////////////////////////////////////////////////////
#include "../../library/thespecialists"

/////////////////////////////////////
// TS_Barrett namespace
namespace TS_Barrett
{
    /////////////////////////////////////
    // Barrett animation enumeration
    ...
    // Meta data
    const string            strNAME                 = "barrett"             ;
    const string            strNAMESPACE            = "TS_Barrett::"        ;
    const string            strCLASSNAME            = "weapon_ts_barrett"   ;
    ...
    const string            strSOUND_CLIPIN         = TheSpecialists::strSOUND_PATH + "heavy/" + strNAME + "/" + TheSpecialists::strHEAVY__SOUND__CLIPIN       ;
    const string            strSOUND_CLIPOUT        = TheSpecialists::strSOUND_PATH + "heavy/" + strNAME + "/" + TheSpecialists::strHEAVY__SOUND__CLIPOUT      ;
    const string            strSOUND_FIRE           = TheSpecialists::strSOUND_PATH + "heavy/" + strNAME + "/" + TheSpecialists::strHEAVY__SOUND__FIRE         ;
    const string            strSOUND_SLIDEBACK      = TheSpecialists::strSOUND_PATH + "heavy/" + strNAME + "/" + TheSpecialists::strHEAVY__SOUND__SLIDEBACK    ;
    ...
    const float             fPRIMARY_ATTACK_DELAY   = TheSpecialists::fWEAPON__BARRETT__ATTACK_DELAY        ;
    const float             fSWING_DISTANCE         = TheSpecialists::fSWING_DISTANCE                       ;
    const IGNORE_MONSTERS   eIGNORE_RULE            = TheSpecialists::eIGNORE_RULE                          ;
    const int               iDAMAGE                 = TheSpecialists::iWEAPON__BARRETT__DAMAGE              ;
    const Vector            vecSPREAD               = TheSpecialists::vecWEAPON__BARRETT__SPREAD            ;
    const float             fRECOIL_MULTIPLIER      = TheSpecialists::fWEAPON__BARRETT__RECOIL_MULTIPLIER   ;
    
    /////////////////////////////////////
    // BARRETT class
    class weapon_ts_barrett : ScriptBasePlayerWeaponEntity
    {
        ...
        private bool    m_bADS                  ; // Aim down sights (ADS) flag
        private float   m_fAttackDelay          ; // Weapon attack delay
        ...
        TraceResult     m_trHit                 ; // Keeps track of what is hit when the barrett is swung
        ...
        void Spawn()
        {
            ...
            // Initialize the ADS flag
            m_bADS = false;
            
            // Initialize the attack delay variable
            m_fAttackDelay = TheSpecialists::fWEAPON__BARRETT__ATTACK_DELAY;
            ...
            // Set the clip size
            self.m_iClip = TheSpecialists::iWEAPON__BARRETT__CLIP;
        ...
        {
            info.iMaxClip		= TheSpecialists::iWEAPON__BARRETT__CLIP    ;
            info.iMaxAmmo1		= TheSpecialists::iWEAPON__BARRETT__AMMO1   ;
            info.iMaxAmmo2		= TheSpecialists::iWEAPON__BARRETT__AMMO2   ;
            info.iSlot			= TheSpecialists::iWEAPON__SLOT__HEAVY      ;
            info.iPosition		= TheSpecialists::iWEAPON__POSITION__BARRETT;
        ...
        bool AddToPlayer(CBasePlayer@ pPlayer)
        {
            ...
            if(bReturn)
            {
                ...
                // Debug printing
                // g_EngineFuncs.ClientPrintf(m_pPlayer, print_console, "BARRETT m_iPrimaryAmmoType: " + self.m_iPrimaryAmmoType + "\n");
        ...
        bool Deploy()
        {
            // Allow the deploy animation to play before idling
        ...
        void Holster(int skiplocal)
        {
            ...
            // Tell the game loop to stop calling any of our barrett functions
            SetThink(null);
        } // End of Holster()
        
        //////////////////////////////////////////////////
        // TS_Barrett::PrimaryAttack                    //
        // Function:                                    //
        ...
        void PrimaryAttack()
        {
            // Fully automatic HEAVY so no need to check for any button presses
            Shoot();
            ...
            self.m_flNextPrimaryAttack  = g_Engine.time + m_fAttackDelay;
            m_flAnimationCooldown       = g_Engine.time + 2.0;
        } // End of PrimaryAttack()

        //////////////////////////////////////////////////
        // TS_Barrett::SecondaryAttack                  //
        // Function:                                    //
        ...
        void SecondaryAttack()
        {
            ...
            if (m_bADS)
            {
                // Player is aiming down sight
                
                // Time to unset the zoom
                SetZoom(TheSpecialists::ZoomType::iZOOM_NONE);
                
                // Reset the weapon's attack RPM to normal
                m_fAttackDelay = TheSpecialists::fWEAPON__BARRETT__ATTACK_DELAY;
            }
            else
            {
                // Player is NOT aiming down sight
                
                // Time to set the zoom
                SetZoom(TheSpecialists::ZoomType::iZOOM_2X);
                
                // Set the fire rate to be slower and more controllable while zooming
                m_fAttackDelay = TheSpecialists::fWEAPON__BARRETT__ATTACK_DELAY__ZOOM;
            }
            
            // Toggle the ADS flag
            m_bADS = !m_bADS;
            
            self.m_flNextSecondaryAttack = g_Engine.time + TheSpecialists::fDEFAULT_SCOPE_DELAY;
        ...
        // TS_Barrett::SetZoom                      //
        // Function:                                //
        //      Sets the weapon zoom                //
        // Parameters:                              //
        //      int iZoom = [IN] Zoom magnification //
        // Return value:                            //
        //      None                                //
        //////////////////////////////////////////////
        void SetZoom(int iZoom)
        {
            m_pPlayer.pev.fov   = iZoom;
            m_pPlayer.m_iFOV    = iZoom;
        } // End of SetZoom()
            ...
            if (m_pPlayer.pev.waterlevel != WATERLEVEL_HEAD)
            {
                ...
                if (self.m_iClip > 0)
                {
                    ...
                    (
                        ...
                        TheSpecialists::iWEAPON__SNIPER__BULLET__TYPE   , // int iBulletType        - Bullet type, not sure what this means
        ...
        void Reload()
        {
            ...
            if (    (self.m_iClip == TheSpecialists::iWEAPON__BARRETT__CLIP)
                 || (m_pPlayer.m_rgAmmo(self.m_iPrimaryAmmoType) <= 0)    )
            {
                return;
            }
            
            self.DefaultReload(TheSpecialists::iWEAPON__BARRETT__CLIP, Animations::RELOAD1, 1.5, 0);
    ...
    } // End of class weapon_ts_barrett
    ...
    void Register_Weapon()
    {
        ...
        (
            ...
            TheSpecialists::strWEAPON__SNIPER__AMMO_TYPE  // string - ammo type
        );
    }
} // End of namespace TS_Barrett